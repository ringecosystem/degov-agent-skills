...
The public response does not include a generation time, data-as-of time, coverage label, readiness
state, or serving revision. A successful response proves that the published view was queryable; it
does not prove source freshness. Verify time-sensitive claims through the returned official source
URL.
...
- `nextCursor` is non-null only when `hasMore` is true. Pass it back unchanged to the same endpoint
  with the same filters, sort, and limit.
- Never inspect, construct, shorten, or reuse a cursor for another request shape.
- A cursor can expire when the published serving view changes. Restart from the first page only if
  more results are still needed.
...
| `cursor` | string | Cursor from the preceding DAO page |

```bash
curl -sS "https://agent-api.degov.ai/v2/daos?limit=25"
```
...
There is no name-search parameter. To resolve a natural-language DAO name, paginate the directory
until an unambiguous `name`/`daoId` match is found; ask the user when multiple matches remain.
...
Lists proposals using exact structured filters.
...
```bash
curl -sS \
  "https://agent-api.degov.ai/v2/proposals?daoId=aavedao-eth&status=active&status=pending&sort=votingEndsAtAsc&limit=10"
```
...
`proposerId`, `discussionUrl`, `status`, and the three timestamps can be `null`. The list has no
title or full-text search. Narrow by DAO/time when possible; otherwise use an official search.
...
Returns the proposal-list fields plus `body` and `choices`.
...
Either field can be `null`. The contract does not currently expose a normalized outcome, quorum,
execution state, voting mechanism, or executable actions; use `source.url` for those claims.
...
`address`, `votingPower`, and `votedAt` can be `null`. `rawChoice` can be any provider JSON value,
including an object. For a scalar, map its string form to proposal-detail `choices[].id`; for a
complex value, do not infer selection or weight semantics that the API does not describe.
...
```bash
curl -sS \
  "https://agent-api.degov.ai/v2/forum-topics?daoId=aavedao-eth&sort=commentCountDesc&limit=5"
```
...
`excerpt`, `category`, and either timestamp can be `null`. There is no topic-detail endpoint. Open
`source.url` when the user asks what the discussion actually says and the excerpt is insufficient.
...
`votingPower` and `votedAt` can be `null`. Interpret `rawChoice` with the same caution as proposal
vote rows. Fetching proposal detail can map a simple scalar to a label, but complex ballots may
remain ambiguous.

## Known contract limits

- No public response field establishes source freshness.
- No DAO-name, proposal-title, or full-text search is available.
- Proposal outcome, quorum, execution, voting mechanism, and executable actions are not exposed.
- Forum topic detail and author are not exposed; excerpts can be absent.
- Vote rows expose provider-shaped `rawChoice`, not a normalized selection, and do not expose a
  transaction hash.
