...
| `400 CURSOR_INVALID`                                                             | Cursor malformed or not parseable                                                     | Drop the cursor; re-run                                                                                                       |
| `404 NOT_FOUND` (key)                                                            | Key expired/versioned away                                                            | Re-resolve via list or `proposals/resolve`                                                                                    |
| `409 CURSOR_STALE`                                                               | Endpoint, limit, filters, or serving revision differ from the cursor                  | Drop the cursor; for a paid route, re-run only within the approved query plan and cost cap                                    |
| `429 RATE_LIMITED` / `QUOTA_EXCEEDED`                                            | Too many requests                                                                     | Back off; ask the user before continuing                                                                                      |
| `500 INTERNAL_ERROR`                                                             | Server issue                                                                          | Retry a free request once; do not automatically repeat a payment; if persistent, report                                       |
