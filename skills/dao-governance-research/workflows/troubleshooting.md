...
| Symptom                                                                          | Cause                                                                   | Fix                                                                                                   |
| -------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `400 VALIDATION_ERROR`                                                           | Bad param/enum/CSV                                                      | Re-read the endpoint card in `api-v2.md`; check `details`                                             |
...
| `414 URI Too Long` on `proposals/:proposalKey` / `forum-topics/:topicKey` routes | Fastify `maxParamLength` default (100) rejects the ~190-char opaque key | Known API-side issue (staging 2026-08); no payment is made. Retry later or use list/resolve endpoints |
...
| Symptom                         | Cause                                                                          | Fix                                                                                                                                       |
| ------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `402` with `PAYMENT-REQUIRED`   | Normal for paid endpoints                                                      | Run the payment ceremony (SKILL.md → Payment ceremony)                                                                                    |
...
| Payment signed but still `402`  | Envelope/settlement mismatch, or authorization expired                         | Check `PAYMENT-RESPONSE`; never re-pay blindly — see `errors.md`                                                                          |
